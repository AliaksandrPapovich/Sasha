import datetime                                                         # подключает время из системы
choose_time = input("Выбери вариант\n1.текущеее время\n2 ваше время\n") #  \n1 переносит на новый обзац
if choose_time == '1':
    current_time = datetime.datetime.now().strftime("%H:%M")            # strftime("%H:%M")  формат времени 
    ...
    time_user = choose_time.split(":")                            #  .split(":")  разделяет вводимые данные чем-нибудь (12:25)
...
minute_dictionary_names = {1: "минут", 2: "минуты", 3: "минута"}       # словарь{ключ: значение}
minutes_1 = minutes%10
minutes_10 = minutes - minutes_1
if 4 < minutes < 21:
    name_minutes = 1                              #  присваиваем ключ
elif minutes_1 > 4:
    name_minutes = 2
elif minutes_1 == 1:   
    name_minutes = 3 
